...
// ==========================================
import coverImg from '@/assets/media/book/cover.jpeg';
import p1 from '@/assets/media/book/pic1.jpeg';
import p2 from '@/assets/media/book/pic2.jpeg';
import p3 from '@/assets/media/book/pic3.jpeg';
import p4 from '@/assets/media/book/pic4.jpeg';
...
<template>
  <div :class="['book', bookReady ? 'visible' : 'hidden']">
    <div ref="bookContainer" class="flipbook-container">
      ...
      >
        <div class="page">
          <div v-if="index === 6" class="page-with-overlay">
            <img :src="pageImg" :alt="`page ${index + 1}`" class="page-img" />
          ...
          <img 
            ...
            :src="pageImg" 
            :alt="`page ${index + 1}`" 
            :class="['page-img', index === 7 ? 'cover-page' : '']" 
          />
        </div>
      </div>
...
// ==========================================
// 🛠️ תמונות וקבצים מעודכנים (8 עמודים)
// ==========================================
import coverImg from '@/assets/media/book/cover.jpeg';
import p1 from '@/assets/media/book/pic1.jpeg';
import p2 from '@/assets/media/book/pic2.jpeg';
...
const handleResize = () => {
  updateDimensions();
  if (pageFlipInstance) {
    pageFlipInstance.updateFromHtml(); // או updateFromHTML בהתאם לספרייה שלך
  }
};
...
onMounted(async () => {
  ...
  // השהייה קלה להתייצבות ה-DOM ב-Vue
  setTimeout(() => {
    if (bookContainer.value) {
      try {
        pageFlipInstance = new PageFlip(bookContainer.value, {
        ...
        const pageElements = bookContainer.value.querySelectorAll('.page-wrapper');
        
        // 🔄 הוחזר ל-loadFromHTML המקורי שעבד אצלך בפרויקט
        pageFlipInstance.loadFromHTML(pageElements);
        
        pageFlipInstance.on('flip', onPageFlip);
        bookReady.value = true;
        
        // עמוד הפתיחה (האינדקס האחרון במערך ההפוך באורך 8)
        pageFlipInstance.turnToPage(7);
      } catch (error) {
        console.error("שגיאה באתחול ה-PageFlip:", error);
      }
    }
